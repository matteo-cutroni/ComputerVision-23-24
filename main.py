...
while cap.isOpened():
    ...
    if success:
        ...
        for i, xywh in enumerate(results[0].boxes.xywh):
            ...
            kmeans_player = KMeans(n_clusters=2, init='k-means++', n_init=1)
        ...
        if len(team_colors) == 0:
            ...
            kmeans_team = KMeans(n_clusters=2, init='k-means++', n_init=1)
            kmeans_team.fit(list(player_color.values()))

            labels = kmeans_team.labels_
        ...
        for i, xywh in enumerate(results[0].boxes.xywh):
            ...
            if player_color[int(results[0].boxes.id[i])] not in team_colors:
                ...
                distances = [np.linalg.norm(player_color[int(results[0].boxes.id[i])] - color) for color in team_colors]
